...
create type public.stock_movement_type as enum ('PURCHASE_ENTRY', 'SALE', 'RETURN', 'EXCHANGE', 'POSITIVE_ADJUSTMENT', 'NEGATIVE_ADJUSTMENT', 'LOSS', 'DAMAGED_PRODUCT', 'TRANSFER');
create type public.cash_movement_type as enum ('OPENING', 'SALE', 'INCOME', 'EXPENSE', 'WITHDRAWAL', 'REFUND', 'CLOSING_ADJUSTMENT');
create type public.inventory_status as enum ('DRAFT', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED');
...
create unique index owners_document_unique on public.owners(business_id, document) where document is not null;
...
create unique index suppliers_tax_id_unique on public.suppliers(business_id, tax_id) where tax_id is not null;
...
create unique index customers_document_unique on public.customers(business_id, document) where document is not null;
...
create unique index one_open_cash_session_per_register on public.cash_sessions(cash_register_id) where status='OPEN';
...
create index profiles_business_idx on public.profiles(business_id, active);
create index products_search_idx on public.products(business_id, active, name);
create index variants_product_idx on public.product_variants(product_id, active);
create index lots_available_fifo_idx on public.inventory_lots(business_id, variant_id, owner_id, received_at) where available_quantity > 0 and active;
create index stock_movements_variant_date_idx on public.stock_movements(business_id, variant_id, created_at desc);
create index purchases_date_idx on public.purchases(business_id, purchased_at desc);
create index sales_date_status_idx on public.sales(business_id, created_at desc, status);
create index sales_cashier_date_idx on public.sales(cashier_id, created_at desc);
create index audit_entity_idx on public.audit_logs(business_id, entity_type, entity_id, created_at desc);
