...
@Component({selector:'app-entity-page',imports:[FormsModule],templateUrl:'./entity-page.html',styleUrl:'./entity-page.css'})
export class EntityPageComponent implements OnInit {
 ...
 async load(){this.loading.set(true);try{const result=await this.data.list<Record<string,unknown>>(this.config().table,{pageSize:100});this.rows.set(result.items);}catch(e){this.toast.show({title:'No se pudieron cargar los datos',description:this.message(e),variant:'danger'});}finally{this.loading.set(false);}}
 ...
 async save(){const cfg=this.config(),value=this.form();if(cfg.fields.some(f=>f.required&&!String(value[f.key]??'').trim())){this.toast.show({title:'Completá los campos obligatorios',variant:'warning'});return;}this.loading.set(true);try{const id=this.editingId();if(id)await this.data.update(cfg.table,id,value);else await this.data.create(cfg.table,{...value,active:true});this.close();await this.load();this.toast.show({title:`${cfg.singular} ${id?'actualizado':'guardado'}`,variant:'success'});}catch(e){this.toast.show({title:'No se pudo guardar',description:this.message(e),variant:'danger'});}finally{this.loading.set(false);}}
 async toggle(row:Record<string,unknown>){const active=Boolean(row['active']);if(!confirm(`¿Confirmás ${active?'desactivar':'activar'} este registro?`))return;try{await this.data.update(this.config().table,String(row['id']),{active:!active});await this.load();}catch(e){this.toast.show({title:'No se pudo actualizar',description:this.message(e),variant:'danger'});}}
 label(field:Field,row:Record<string,unknown>){const value=row[field.key];return value===null||value===''?'—':String(value);}
 private message(e:unknown){return e instanceof Error?e.message:'Verificá la conexión con Supabase.';}
}
